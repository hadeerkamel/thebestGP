...
%figure,imshow(compIDilated);
 ...
 for i=1:N
     subImages{i} = compIDilated;
     for j = 1:N
     if(j~=i)
     subImages{i}(CC.PixelIdxList{j}) = 0;
     end
     end
 end
 indd = 1;
 NonrelevantConnCompIndices = [];
for nComp = 1:N 
%figure,imshow(subImages{nComp});
[H W] = size(compIDilated);     
index = 1;
 for r = 1:H
     for c = 1:W
         if(subImages{nComp}(r,c)== 1)
            rPixelList(index) = r;
            cPixelList(index) = c;
            index = index + 1;
         end
...
% 
 HH = (maxY - minY);
 WW = maxX - minX;
newSubImage{nComp} = imcrop(compIDilated,[minX minY W HH]);



%check if isolated conn comp remove


[HH W] = size(newSubImage{nComp});
%figure,imshow(newSubImage{nComp});
for r = 1:HH
     whitecount = 0;
     blackCount = 0;
    for c = 1:W
        if(newSubImage{nComp}(r,c)==1)
    ...
    WhitesPerRow(r) = whitecount;
    BlacksPerRow(r) = blackCount;
end
...
if(R<60)
    NonrelevantConnCompIndices(indd) = nComp;
    indd = indd + 1;
end
end   
%figure,imshow(compIDilated);
CC = bwconncomp(compIDilated);
N = CC.NumObjects;
L = length(NonrelevantConnCompIndices);
for i = 1:N
    for ind = 1:L
        if(i==NonrelevantConnCompIndices(ind))
            compIDilated(CC.PixelIdxList{i}) = 0;
        end
    end
end
%figure,imshow(compIDilated);
